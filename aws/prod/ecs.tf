...
resource "aws_service_discovery_service" "team_discovery" {
  ...
  name = "${each.value.team}-${index(local.team_ecs_configs[each.value.team].ecs_instances, each.value.instance)}-service"
  dns_config {
...
resource "aws_ecs_service" "team_services" {
  ...
  load_balancer {
    ...
    container_name   = "${each.value.team}-${index(local.team_ecs_configs[each.value.team].ecs_instances, each.value.instance)}-container"
    container_port   = 8080
  }
...
resource "aws_ecs_task_definition" "team_tasks" {
  ...
  family                   = "${each.value.team}-${index(local.team_ecs_configs[each.value.team].ecs_instances, each.value.instance)}-task-definition"
  ...
  container_definitions = jsonencode([{
    name      = "${each.value.team}-${index(local.team_ecs_configs[each.value.team].ecs_instances, each.value.instance)}-container"
    image     = "${module.team_ecr[each.value.team].repository_url}:latest"
    cpu       = each.value.instance.vCpu
    memory    = each.value.instance.memory
    essential = true
    ...
    # Rest of the container definition remains the same
  }])
...
resource "aws_cloudwatch_log_group" "team_logs" {
  ...
  name              = "/aws/ecs/${each.value.team}/${index(local.team_ecs_configs[each.value.team].ecs_instances, each.value.instance)}"
  retention_in_days = 7
